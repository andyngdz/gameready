...
use super::*;
use crate::games::{GameProfile, Source, Wrapper};
...
fn a_game_with_wrappers_becomes_a_target() {
    ...
    assert_eq!(targets[0].name, "Deadlock");
}

#[test]
fn a_game_with_no_profile_is_left_out() {
    assert!(targets_for(&[setup("Hollow Knight", 367_520, None)]).is_empty());
}
