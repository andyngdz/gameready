...
use gameready_core::rollback::{RollbackPlan, RollbackReport, UndoOutcome, UndoReport};

use crate::cli::ui::layout::{Mark, ResultTable, Section};
use crate::cli::ui::{theme, widest};
use crate::features::{preview_rows, PreviewRow};

const CONFIRM_STEAM_CLOSE: &str = "Yes, close Steam and undo";
...
pub fn confirm_steam_close() -> Result<bool> {
    ...
    .one_of(vec![CONFIRM_STEAM_CLOSE, "No, cancel rollback"])
    .prompt_skippable()?;
    Ok(choice == Some(CONFIRM_STEAM_CLOSE))
}
